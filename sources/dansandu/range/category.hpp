...
struct is_pipe_head_wrapper {
    template<typename = Type>
    static constexpr auto getValue(general_case) {
        return false;
    }
    ...
    static constexpr auto getValue(best_case) {
        return value;
    }
};
...
struct is_pipe_head_wrapper<std::vector<Type>> {
    static constexpr auto getValue(best_case) { return true; }
};

template<typename Type>
inline constexpr auto is_pipe_head = is_pipe_head_wrapper<typename std::decay_t<Type>>::getValue(best_case{});
